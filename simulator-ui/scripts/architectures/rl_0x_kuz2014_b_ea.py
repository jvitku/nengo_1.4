...
def buildSimulation(alpha, gamma, lambdaa, importance, expName='test0', indNo=1):
	...
	# connect world to the RL node, connect XY, not Reward directly
...
def configure(genome):
    ...
    #alphaIn.setFunctions([ConstantFunction(1,   genome[16])]) #dimension, const. value
    #gammaIn.setFunctions([ConstantFunction(1,   genome[17])])
    #lambdaIn.setFunctions([ConstantFunction(1,  genome[18])])
