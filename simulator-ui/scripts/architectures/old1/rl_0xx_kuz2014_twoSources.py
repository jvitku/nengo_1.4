# Run the simulation 10x for given hardwired parameters and write values to separate files
# 
# The same as rl_05, but here, two kinds of rewards are received from te environment, 
# two motivation sources and two RL engines are available
#
...
def buildSimulation(alpha, gamma, lambdaa, importance, expName='test0', indNo=1):
	net=nef.Network('HardwiredModel01 oneReward + Two MotivationSources')
	net.add_to_nengo()  
	...
	world = gridworld.benchmarkTwoR(name="map_20x20", mapName = "BenchmarkGridWorldNodeD", logPeriod=10000)
	...
	tx=[[0 for j in range(3)] for i in range(4)]
	tx[2][1] = 1;
	tx[3][2] = 1;	# identity transform without first TWO dimensions (do not connect rewards directly!)
	...
	ty=[[0 for j in range(1)] for i in range(4)] # yx (y is second dim)
	ty[0][0] = 1;
