# The same as rl_14 but here, the GA is used.
#
...
from design.ea.algorithm.impl import BinaryVectorEA
...
def addModel(net, modelName='model0'):
    ...
    world             = gridworld.benchmarkA(modelName+"_map_15x15",mapName="BenchmarkGridWorldNodeD",logPeriod=100000,synchronous=False);
    source            = motivation.basic(modelName, 1, Motivation.DEF_DECAY, logPeriod=10000,synchronous=False) 
...
def evalInd(indNo, gen, genome, model):
	...
	prosperity = (prosp[0]*prospMo[0])
	print 'prosperity-------------------------- '+str(prosperity)+'   (prosperity of RL '+str(prosp[0])+' ) prosp of Phys '+str(prospMo[0])
...
FOLDER = 'generated-data/cmoea/GA_data'		# make this folder under the simulator-ui
...
for i in range(noExperiments):
	FOLDER = 'generated-data/cmoea/GA_exp_'+str(i)
	#modelA[3].setFolderName(FOLDER)
	ea = BinaryVectorEA(genomelen, False, maxGens, popSize);
	...
	while(ea.wantsEval()):
		...
		prosp = evalInd(indNo, gen, genome, modelA)
		#prosp  = random.random()
...
configure(best.getGenome().getVector(),modelA)  # load the best solution
net.view()                                      # shove it
