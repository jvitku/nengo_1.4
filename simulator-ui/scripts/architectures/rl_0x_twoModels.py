...
class ProsperitySaverb(nef.SimpleNode):
	def init(self):
		self.rlp = [0,0,0];
	def termination_RLprosperity(self, values, dimensions=3):
		self.rlp = values;
	def tick(self):
		...
		#print rl.getOrigin(QLambda.topicProsperity).getValues().getValues(); 	# composed prosp of rl
		#print mo.getOrigin(Motivation.topicProsperity).getValues().getValues();	# 1-MSD
		ppp = rlb.getOrigin(QLambda.topicProsperity).getValues().getValues();
		pp = mob.getOrigin(Motivation.topicProsperity).getValues().getValues()
		#f=file(self.name,'a+')
		f=file(expNAMEEb,'a+')
		#f.write('%1.3f %s %s\n'%(self.t,str(pp[0]),' '.join(map(str, self.rlp))))
		f.write('%1.3f %s %s\n'%(self.t, str(pp[0]),' '.join(map(str, ppp))))
...
net = buildSimulationb(QLambda.DEF_ALPHA,QLambda.DEF_GAMMA,QLambda.DEF_LAMBDA,0,name) 
rl = net.get("RL_QLambda")
mo = net.get("BasicMotivation_Motivation")

for i in range(10):
	expNAMEE = "expmane"+str(i)+"_a.txt"
	expNAMEEb = "expmane"+str(i)+"_b.txt"
	net.reset()
	time.sleep(2)
	net.run(t,dt)
