...
encoder_A = [random.choice([-1,1]) for i in range(N_A)] #for each neuron +-1
encoder_B = [random.choice([-1,1]) for i in range(N_B)]

# define tunning curves, parameters:
# num neurons
# intercepts: min and max where it can cross the X axis
# rates: min and max rate (respondint (I guess) for input of value 1)
