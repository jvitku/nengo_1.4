...
title='FuzzyNOT'
label='FuzzyNOT'
icon='logic_fuzzy_gates_not.png'

# parameters for initializing the node
params=[
('name','Select name for NeuralModule FuzzyNOT',str),
...
def make(net,name='NeuralModule which implements logical Fuzzy NOT operation', 
independent=True, useQuick=True):

    finder = "org.hanns.logic.fuzzy.gates.impl.NOT";

    # create group with a name
    g = NodeGroup(name, independent);    	# create independent group called..
    g.addNode(finder, "logic_fuzzy_gates_NOT", "java");      # start java node and name it finder
    neuron = NeuralModule(name+"_logic_fuzzy_gates_not", g) # construct the neural module 
